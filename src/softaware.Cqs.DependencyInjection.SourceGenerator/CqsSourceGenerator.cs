...
public class CqsSourceGenerator : IIncrementalGenerator
{
        ...
        CancellationToken ct)
    {
        ...
        foreach (var invocation in invocations)
        {
            ...
            if (arg is TypeOfExpressionSyntax typeOfExpr)
            {
                ...
                if (typeInfo.Type is INamedTypeSymbol namedType)
                {
                    config.DecoratorTypes.Add(namedType.OriginalDefinition);
                }
            }
            else
            {
        ...
        SourceProductionContext context)
    {
        ...
        {
            foreach (var decoratorType in mergedConfig.DecoratorTypes)
            {
                if (DecoratorApplies(compilation, decoratorType, handler.RequestType, handler.ResultType, requestHandlerType))
                {
                    handler.ApplicableDecorators.Add(decoratorType);
                }
            }
        }
    ...
    private static string GenerateRegistrationCode(List<HandlerInfo> handlers, Compilation compilation)
    {
        ...
        foreach (var handler in handlers)
        {
            ...
            else
            {
                ...
                foreach (var decorator in handler.ApplicableDecorators)
                {
                    var closedDecoratorFqn = GetClosedDecoratorName(decorator, handler.RequestType, handler.ResultType);
